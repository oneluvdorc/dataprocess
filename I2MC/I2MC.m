...
opt.xres                        = 1366; % maximum value of horizontal resolution in pixels
opt.yres                        = 768; % maximum value of vertical resolution in pixels
...
opt.freq                        = 120; % sampling frequency of data (check that this value matches with values actually obtained from measurement!)

% Variables for the calculation of visual angle
% These values are use     d to calculate noise measures (RMS and BCEA) of
% fixations. The may be left as is, but don't use the noise measures then.
opt.scrSz                       = [34.5, 19.4]; % screen size in cm
opt.disttoscreen                = 60; % distance to screen in cm.
...
folders.data                    = 'input'; % folder in which data is stored (each folder in folders.data is considered 1 subject)
...
opt.downsamples                 = [2 5];
opt.downsampFilter              = 1;          % use chebychev filter when downsampling? 1: yes, 0: no. requires signal processing toolbox. is what matlab's downsampling functions do, but could cause trouble (ringing) with the hard edges in eye-movement data
opt.chebyOrder                  = 5;        % order of cheby1 Chebyshev downsampling filter, default is normally ok, as long as there are 25 or more samples in the window (you may have less if your data is of low sampling rate or your window is small
% 
...
% opt.maxMergeTime                = 40; % maximum time in ms between fixations for merging
